...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        embeddig_lookup = self.pretrained_embeddings
        embeddings = tf.nn.embedding_lookup (
            embeddig_lookup, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(init((self.config.n_features*self.config.embed_size, self.config.hidden_size)))
        b1 = tf.Variable(tf.zeros([self.config.hidden_size]))
        U = tf.Variable(init((self.config.hidden_size, self.config.n_classes)))
    ...
    def add_loss_op(self, pred):
        ...
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred, 
                                                                      labels=self.labels_placeholder), axis=None)
        ### END YOUR CODE
        return loss
